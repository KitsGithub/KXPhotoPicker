...
- (void)displayImage {
    ...
    opt.synchronous = YES;
    [[PHCachingImageManager defaultManager] requestImageForAsset:self.asset targetSize:CGSizeMake([UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height) contentMode:PHImageContentModeAspectFit options:opt resultHandler:^(UIImage * _Nullable result, NSDictionary * _Nullable info) {
        if (result) {
            self.photoImageView.image = result;
            CGRect photoImageViewFrame;
            photoImageViewFrame.size = CGSizeMake(result.size.width, result.size.height);
